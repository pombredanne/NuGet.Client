...
namespace NuGet.Packaging.Signing
{
    ...
    {
        internal const string NU3042HelpUrl = "https://aka.ms/nuget-cross-platform-verification";
        ...
        private ILogMessage GetAdditionalContext(X509Chain chain)
        {
            ...
            // If a certificate chain is untrusted simply because the root certificate is untrusted (versus clearly
            // intentionally distrusted), then create an additional message explaining how one might resolve this lack
            // of trust.
            if (root.ChainElementStatus.Any(status => status.Status.HasFlag(X509ChainStatusFlags.UntrustedRoot)) &&
                !root.ChainElementStatus.Any(
                    status => status.Status.HasFlag(X509ChainStatusFlags.Revoked) || status.Status.HasFlag(X509ChainStatusFlags.ExplicitDistrust)))
            {
